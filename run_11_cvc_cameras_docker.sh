...
start_cam() {
    ...
    tmux new-session -d -s "$SESSION_NAME" \
        "export CUDA_DEVICE_ORDER=PCI_BUS_ID && export CUDA_VISIBLE_DEVICES=$DEVICE && python3 historical-processor/orchestrator.py \
            --folder-id '$FOLDER_ID' \
...
echo "All cameras launched in tmux sessions!"

# Keep container alive and stream tmux logs or just sleep
# In docker, we must keep the main process running.
echo "Press Ctrl+C to stop. Container will run indefinitely."
sleep infinity
