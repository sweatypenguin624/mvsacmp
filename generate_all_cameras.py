...
for loc, f_id in roots.items():
    ...
    for d in dirs:
        if d.get("IsDir") and "backup" not in d.get("Name", "").lower():
            cam_id = d["ID"]
            cam_name = d["Name"].replace(" ", "_").replace("-", "").lower()
            final_name = f"{loc}_{cam_name}"
            
            remote = f"Gdrive-yogesh,root_folder_id={cam_id}:"
            print(f"Finding first .dav file in {final_name}...")
            c2 = [RCLONE, "--config", "config/rclone.conf", "lsf", "-R", "--include", "*.dav", remote]
            r2 = subprocess.run(c2, capture_output=True, text=True)
            lines = r2.stdout.strip().split("\n")
            dav_file = lines[0] if lines and lines[0] else None
            
            if dav_file:
                cameras_to_run.append((final_name, cam_id))
                out_img = os.path.join(OUT_DIR, f"{final_name}.jpg")
                print(f"Extracting frame to {out_img}...")
...
print("Creating bash script run_11_cvc_cameras.sh...")
bash_script = "#!/bin/bash\nset -e\ncd /home/users/oauser/mvsa\n\nstart_cam() {\n"
bash_script += """    local SESSION_NAME="$1"
    local FOLDER_ID="$2"
    local CAM_NAME="$3"
    local DEVICE="$4"
...
bash_script += "echo \"All cameras launched!\"\n"
with open("run_11_cvc_cameras.sh", "w") as f:
    f.write(bash_script)
os.chmod("run_11_cvc_cameras.sh", 0o755)
print("Done!")
